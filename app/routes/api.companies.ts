...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const last_funding_type =
    url.searchParams.get("last_funding_type") ?? undefined;
  const min_rank = url.searchParams.get("min_rank")
    ? parseInt(url.searchParams.get("min_rank")!, 10)
    : undefined;
  const max_rank = url.searchParams.get("max_rank")
    ? parseInt(url.searchParams.get("max_rank")!, 10)
    : undefined;
  const min_funding = url.searchParams.get("min_funding")
    ? parseInt(url.searchParams.get("min_funding")!, 10)
    : undefined;
